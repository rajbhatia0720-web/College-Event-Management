...
function isPlayableVideo(video) {
  return video.source === 'device' || isDirectVideo(video.url) || video.url.startsWith('data:video/');
}
...
function renderVideoCard(video, isAdmin = false) {
  ...
  const preview = embedUrl ? `<iframe src="${embedUrl}" title="${video.title}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>` : isPlayableVideo(video) ? `<video src="${video.url}" controls preload="metadata"></video>` : `<div class="video-fallback"><span>▶</span><small>Open video link</small></div>`;
  const sourceLabel = video.source === 'device' ? 'FROM DEVICE' : video.source === 'direct' || isDirectVideo(video.url) ? 'DIRECT VIDEO' : video.source === 'external' ? 'VIDEO LINK' : 'YOUTUBE';
  return `<article class="video-card"><div class="video-preview">${preview}</div><div class="video-card-content"><div class="video-card-meta"><span class="category-pill technical">${video.category.toUpperCase()}</span><span>${sourceLabel} · ${relatedEvent ? relatedEvent.date : 'CAMPUS'}</span></div><h3>${video.title}</h3><p>${video.description || 'A new CampusLoop video connected to this event.'}</p><div class="video-card-footer"><span>◈ ${video.event}</span><span class="video-actions"><a href="${video.url}" target="_blank" rel="noreferrer">Open ↗</a>${isAdmin ? `<button class="small-button danger-button delete-video" data-video="${video.title}">Delete</button>` : ''}</span></div></div></article>`;
}
...
$('#new-video-source').addEventListener('change', () => { const isDevice = $('#new-video-source').value === 'device'; $('#device-upload-field').classList.toggle('hidden', !isDevice); $('#new-video-url').required = !isDevice; $('#new-video-file').required = isDevice; $('#video-source-hint').textContent = isDevice ? 'Choose a local video file. It will be stored on this browser for the demo.' : 'YouTube links embed automatically. Direct .mp4 links play in the app; other links open in a new tab.'; });
$('#new-video-file').addEventListener('change', () => { const file = $('#new-video-file').files[0]; const preview = $('#device-preview'); if (!file) { preview.classList.add('hidden'); return; } if (!file.type.startsWith('video/') || file.size > 10 * 1024 * 1024) { showToast('Choose a video file smaller than 10 MB'); $('#new-video-file').value = ''; preview.classList.add('hidden'); return; } preview.innerHTML = `<span>✓</span> ${file.name} · ${(file.size / 1024 / 1024).toFixed(1)} MB`; preview.classList.remove('hidden'); });
$('#create-video-form').addEventListener('submit', (event) => { event.preventDefault(); const source = $('#new-video-source').value; const file = $('#new-video-file').files[0]; if (source === 'device' && !file) { showToast('Choose a video from your device first'); return; } const publish = (url) => { state.videos.unshift({ title: $('#new-video-title').value, event: $('#new-video-event').value, category: $('#new-video-category').value, source, url, description: $('#new-video-description').value || 'A new CampusLoop video connected to this event.' }); try { localStorage.setItem('campusloop-videos', JSON.stringify(state.videos)); } catch { state.videos.shift(); showToast('This video is too large for browser storage'); return; } event.target.reset(); $('#device-upload-field').classList.add('hidden'); $('#new-video-url').required = true; $('#new-video-file').required = false; renderAdminVideos(); renderVideos(); showToast('Video published to the event hub'); }; if (source === 'device') { const reader = new FileReader(); reader.onload = () => publish(reader.result); reader.readAsDataURL(file); } else publish($('#new-video-url').value); });
