...
function renderVideoCard(video, isAdmin = false) {
  ...
  const preview = embedUrl ? `<iframe src="${embedUrl}" title="${video.title}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>` : isDirectVideo(video.url) ? `<video src="${video.url}" controls preload="metadata"></video>` : `<div class="video-fallback"><span>▶</span><small>Open video link</small></div>`;
  const sourceLabel = video.source === 'direct' || isDirectVideo(video.url) ? 'DIRECT VIDEO' : video.source === 'external' ? 'VIDEO LINK' : 'YOUTUBE';
...
$('#create-video-form').addEventListener('submit', (event) => { event.preventDefault(); state.videos.unshift({ title: $('#new-video-title').value, event: $('#new-video-event').value, category: $('#new-video-category').value, source: $('#new-video-source').value, url: $('#new-video-url').value, description: $('#new-video-description').value || 'A new CampusLoop video connected to this event.' }); localStorage.setItem('campusloop-videos', JSON.stringify(state.videos)); event.target.reset(); renderAdminVideos(); renderVideos(); showToast('Video published to the event hub'); });
